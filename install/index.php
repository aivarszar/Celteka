<?php
/**
 * Marketplace Platform - Instalācijas Vednis
 * Automātiska servera pārbaude un datubāzes uzstādīšana
 */
...
define('CONFIG_FILE', INSTALL_DIR . '/app/config/config.php');
...
function testDatabaseConnection($host, $dbname, $username, $password) {
    try {
        ...
        // Pārbaudīt vai datubāze eksistē, ja nē - izveidot
...
function installDatabase($host, $dbname, $username, $password) {
    try {
        ...
        // Izpildīt schema.sql
        $schema = file_get_contents(INSTALL_DIR . '/database/schema.sql');
        $pdo->exec($schema);

        // Izpildīt locations_lv.sql
        $locations = file_get_contents(INSTALL_DIR . '/database/locations_lv.sql');
...
<?php
/**
 * Marketplace Platform - Konfigurācijas Fails
 * Automātiski ģenerēts instalācijas laikā
 */
...
return [
    ...
    'app' => [
        ...
        'url' => 'http' . (isset(\$_SERVER['HTTPS']) ? 's' : '') . '://' . (\$_SERVER['HTTP_HOST'] ?? 'localhost'),
...
if ($step === 'check') {
    // 1. solis: Servera pārbaude
    if (!checkPhpVersion()) {
        $errors[] = "PHP versija ir pārāk veca. Nepieciešama versija " . MIN_PHP_VERSION . " vai jaunāka. Pašreizējā versija: " . PHP_VERSION;
    } else {
    ...
    if (!empty($missingExt)) {
        $errors[] = "Trūkst šādi PHP paplašinājumi: " . implode(', ', $missingExt);
    } else {
        $success[] = "Visi nepieciešamie PHP paplašinājumi ir instalēti ✓";
    }

    $notWritable = checkWritePermissions();
    if (!empty($notWritable)) {
        $warnings[] = "Šīm direktorijām nav rakstīšanas tiesību: " . implode(', ', $notWritable);
    } else {
        $success[] = "Visām direktorijām ir rakstīšanas tiesības ✓";
    }

} elseif ($step === 'database' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    // 2. solis: Datubāzes iestatīšana
    ...
    if (empty($dbname) || empty($username)) {
        $errors[] = "Lūdzu aizpildiet visus obligātos laukus!";
    } else {
        // Testēt savienojumu
        ...
        if ($testResult['success']) {
            ...
            if ($installResult['success']) {
                // Izveidot config failu
                if (createConfigFile($host, $dbname, $username, $password)) {
                    $_SESSION['install_success'] = true;
                    header('Location: ?step=complete');
                    exit;
                } else {
                    $errors[] = "Neizdevās izveidot konfigurācijas failu. Pārbaudiet rakstīšanas tiesības!";
                }
            } else {
                $errors[] = "Datubāzes instalācijas kļūda: " . $installResult['error'];
            }
        } else {
            $errors[] = "Datubāzes savienojuma kļūda: " . $testResult['error'];
        }
    }
}
...
<head>
    ...
    <title>Instalācijas Vednis - Vietējais Tirgus</title>
    <style>
        ...
        <div class="content">
            <?php if ($alreadyInstalled && $step !== 'complete'): ?>
                <div class="alert alert-warning">
                    ⚠️ Platforma jau ir instalēta! Ja vēlaties pārinstalēt, vispirms izdzēsiet failu: <code><?= CONFIG_FILE ?></code>
                </div>
                <div class="text-center">
                    <a href="../public/" class="btn">Doties uz platformu</a>
                </div>
            <?php else: ?>
                <div class="progress">
                ...
                <?php if ($step === 'check'): ?>
                    <div class="text-center">
                        <a href="?step=database" class="btn" <?= !empty($errors) ? 'style="display:none"' : '' ?>>
                            Turpināt uz datubāzes iestatīšanu →
                        </a>
                        <?php if (!empty($errors)): ?>
                            <p style="color: #c33; margin-top: 20px;">
                                Lūdzu novērsiet visas kļūdas pirms turpināt instalāciju.
                            </p>
                        <?php endif; ?>
                    </div>
                ...
                <?php elseif ($step === 'database'): ?>
                    <form method="post" action="?step=database">
                        ...
                        <div class="text-center">
                            <button type="submit" class="btn">Instalēt datubāzi</button>
                        </div>
                    </form>

                <?php elseif ($step === 'complete'): ?>
                    <div class="alert alert-success">
                        <strong>🎉 Instalācija pabeigta veiksmīgi!</strong>
                        <p style="margin-top: 10px;">Platforma ir gatava lietošanai.</p>
                    </div>

                    <div class="alert alert-info">
                        <strong>📝 Nākamie soļi:</strong>
                        <ol style="margin-left: 20px; margin-top: 10px;">
                            <li>Izveidojiet administratora kontu</li>
                            <li>Konfigurējiet platformas iestatījumus</li>
                            <li>Pievienojiet kategorijas un produktus</li>
                        </ol>
                    </div>
                    ...
                    <div class="alert alert-warning" style="margin-top: 20px;">
                        ...
                        <ul style="margin-left: 20px; margin-top: 10px;">
                            <li>Izdzēsiet vai pārdēvējiet <code>/install</code> direktoriju</li>
                            <li>Iestatiet <code>debug => false</code> ražošanas vidē</li>
                            <li>Konfigurējiet HTTPS savienojumu</li>
                        </ul>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
